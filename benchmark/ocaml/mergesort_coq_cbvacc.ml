...
| ys::stack0 ->
  (match ys with
   ...
   | _::_ ->
     (match stack0 with
      | [] -> []::((RevmergeAcc.revmerge leT ys xs)::stack0)
      | zs::stack1 ->
        (match zs with
         | [] -> []::((RevmergeAcc.revmerge leT ys xs)::stack1)
         | _::_ ->
           []::([]::(merge_sort_push leT
                      (RevmergeAcc.revmerge (fun x y -> leT y x)
                        (RevmergeAcc.revmerge leT ys xs) zs) stack1)))))
  ...
  (match ys with
   ...
   | _::_ ->
     ...
     then merge_sort_pop leT false
            (RevmergeAcc.revmerge (fun x y -> leT y x) xs ys) stack0
     else merge_sort_pop leT true (RevmergeAcc.revmerge leT ys xs) stack0)

...
sortN cmp = sort_rec (0 :: Int) [] where
  merge [] ys = ys
  merge xs [] = xs
  ...
  sort_rec k stack (x : y : s)
    ...
    where
      ...
      ascending accu x s =
        let !k' = k + 1 in
        let !xs = accu [x] in
        let !stack' = push xs k stack in sort_rec k' stack' s

      descending accu x [] = pop (x : accu) stack
      descending accu x (y : s) | cmp x y == GT = descending (x : accu) y s
      descending accu x s =
        let !k' = k + 1 in
        let !stack' = push (x : accu) k stack in sort_rec k' stack' s
  sort_rec k stack s = pop s stack

sort3N :: (a -> a -> Ordering) -> [a] -> [a]
sort3N cmp = sort_rec (0 :: Int) [] where
  merge [] ys = ys
  merge xs [] = xs
  merge xs@(x : xs') ys@(y : ys')
    | cmp x y == GT = y : merge xs ys'
  ...
  sort_rec k stack (x : y : z : s)
    ...
    | cmp x y /= GT, cmp y z == GT =
      ...
      let !stack' = push xyz k stack in sort_rec k' stack' s
    | cmp x y == GT, cmp y z /= GT =
      let !k' = k + 1 in

...
- `AppendicesAB.v` provides:
  ...
  MathComp library and `stablesort.v`; that is, this file has a significant
  overlap with them. Nevertheless, this file is supposed to serve as a
  self-contained index of the definitions and lemmas listed in Appendices A and
  B.
...
- `AppendixD.v` provides the definitions of structurally-recursive mergesort
  functions presented in Appendix D, which are copies of the implementations
  (excluding proofs) of mergesort functions in `stablesort.v`, with some
  stylistic modifications, e.g., replacing `seq` with `list`, and adding
  `{struct ..}` annotations.
